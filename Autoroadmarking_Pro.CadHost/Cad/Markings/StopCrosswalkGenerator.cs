...
namespace Autoroadmarking_Pro.CadHost.Cad.Markings
{
    ...
    public sealed class StopCrosswalkGenerator
    {
                ...
                try
                {
                    foreach (string polygonHandle in state.IntersectionPolygonHandles)
                    {
                        ...
                        foreach (double boundaryStation in boundaryStations)
                        {
                            ...
                            // CrosswalkStation vẫn dùng mốc tiếp tuyến này để giữ nguyên contract hiện tại
                            // (đặc biệt khoảng cách cấu hình 7.3 -> 7.1), chỉ thay đổi phạm vi tạo hình 7.3.
                            double crosswalkCenterStation = ResolveBullhornEndStation(
                                db, state, axis, comparison, polygon, tr, boundaryStation, outwardSign, out string anchorSource);

                            double crosswalkStartStation = crosswalkCenterStation;
                            double crosswalkOuterEdgeStation = crosswalkCenterStation + outwardSign * crossingLength;

                            // Giữ nguyên logic 7.1: khoảng cách cấu hình vẫn tính từ mốc 7.3 hiện tại.
                            double stopStation = crosswalkCenterStation + outwardSign * distance;
        ...
        private sealed class BullhornTangencyCandidate
        {
            public double Station { get; set; }
            public double Offset { get; set; }
            public double Alignment { get; set; }
            public double OutwardDistance { get; set; }
            public int SegmentIndex { get; set; }
            public bool AtStart { get; set; }
            public string SourceHandle { get; set; } = string.Empty;
        }

        private sealed class RawEdgeTangencyCandidate
        {
            public double Station { get; set; }
            public double Offset { get; set; }
            public double AxisAlignment { get; set; }
            public double ArcLineAlignment { get; set; }
            public double OutwardDistance { get; set; }
            public string SourceHandle { get; set; } = string.Empty;
            public int SourceSegmentIndex { get; set; } = -1;
        ...
        /// <summary>
        /// Tìm mốc 7.3 trực tiếp trên MÉP CAD nguồn.
        ///
        /// Nguyên tắc:
        /// - BoundaryRunsV3 chỉ ra chính xác Handle + segment của MÉP thật đã tạo sừng bò.
        /// - Với Polyline, chỉ vertex nơi segment BULGE/ARC tiếp giáp segment LINE mới là ứng viên.
        /// - Tiếp tuyến ARC phải gần song song với LINE tại vertex (tiếp tuyến hình học thật).
        /// - LINE sau sừng bò phải gần song song với TIM của approach; điều này loại đầu cung
        ///   phía đường giao cắt trong nút xiên/T/X.
        /// - Mốc cuối cùng lấy phía NGOÀI NHẤT giữa hai mép để toàn bộ 7.3 nằm trên đoạn thẳng.
        ///
        /// Polygon helper không tham gia quyết định station nếu hàm này tìm được dữ liệu hợp lệ.
        /// </summary>
        private bool TryResolveRawCadBullhornTangencyStation(
            Database db,
            ArmProjectState state,
            Entity axis,
            ArmComparisonState comparison,
            ...
            out string source)
        {
            station = boundaryStation;
            source = string.Empty;

            List<BoundaryRunInfo> bullhornRuns = ReadBoundaryRuns(polygon, tr)
                .Where(x => x.Role.Equals("BULLHORN_EDGE", StringComparison.OrdinalIgnoreCase) &&
                            !string.IsNullOrWhiteSpace(x.SourceHandle))
                .ToList();

            var sourceSegments = new Dictionary<string, HashSet<int>>(StringComparer.OrdinalIgnoreCase);
            foreach (BoundaryRunInfo run in bullhornRuns)
            {
                if (!sourceSegments.TryGetValue(run.SourceHandle, out HashSet<int>? set))
                {
                    set = new HashSet<int>();
                    sourceSegments[run.SourceHandle] = set;
                }
                if (run.SourceSegmentIndex >= 0)
                    set.Add(run.SourceSegmentIndex);
            }

            // Polygon cũ chưa có BoundaryRunsV3: vẫn ưu tiên BullhornHandles của polygon,
            // sau đó scanner trên Polyline nguồn sẽ tự tìm segment bulge phù hợp.
            if (sourceSegments.Count == 0)
            {
                ArmEntityMetadata? polygonMd = _metadata.Read(polygon, tr);
                if (polygonMd?.Extra != null &&
                    polygonMd.Extra.TryGetValue("BullhornHandles", out string? rawHandles) &&
                    !string.IsNullOrWhiteSpace(rawHandles))
                {
                    foreach (string handle in rawHandles.Split(new[] { ',', ';' }, StringSplitOptions.RemoveEmptyEntries))
                    {
                        string clean = handle.Trim();
                        if (!string.IsNullOrWhiteSpace(clean) && !sourceSegments.ContainsKey(clean))
                            sourceSegments[clean] = new HashSet<int>();
                    }
                }
            }
            ...
            List<string> connectionHandles = BuildCrosswalkEdgeCandidates(state, polygon, tr, comparison);
            foreach (string handle in sourceSegments.Keys)
            {
                if (!connectionHandles.Contains(handle, StringComparer.OrdinalIgnoreCase))
                    connectionHandles.Add(handle);
            }

            var candidates = new List<RawEdgeTangencyCandidate>();
            foreach (KeyValuePair<string, HashSet<int>> pair in sourceSegments)
            {
                Curve? sourceCurve = TryOpenCurveByHandle(db, tr, pair.Key);
                if (sourceCurve == null)
                    continue;

                if (sourceCurve is Polyline sourcePolyline)
                {
                    CollectPolylineArcToLineTransitions(
                        db,
                        tr,
                        axis,
                        comparison,
                ...
                else if (sourceCurve is Arc sourceArc)
                {
                    CollectStandaloneArcToLineTransitions(
                        db,
                        tr,
                        axis,
                        comparison,
            ...
            // Lọc chuẩn tiếp tuyến. 0.94 tương đương lệch khoảng 20 độ; dữ liệu CAD chuẩn
            // thường > 0.995. Ngưỡng mềm chỉ nhằm chịu sai số polyline/proxy nhỏ.
            List<RawEdgeTangencyCandidate> accepted = candidates
                .Where(x => x.ArcLineAlignment >= 0.94 && x.AxisAlignment >= 0.90)
                .ToList();

            if (accepted.Count == 0)
            {
                double best = candidates.Max(x => Math.Min(x.ArcLineAlignment, x.AxisAlignment));
                accepted = candidates
                    .Where(x => Math.Min(x.ArcLineAlignment, x.AxisAlignment) >= Math.Max(0.82, best - 0.025))
                    .ToList();
            }
            ...
            // Mỗi phía đường chỉ giữ transition gần boundary nhất nhưng không nằm sâu vào nút.
            // Sau đó lấy transition ngoài nhất của hai phía. Điều này xử lý cả trường hợp
            // hai sừng bò có station tiếp tuyến không hoàn toàn bằng nhau.
            double sideTol = Math.Max(0.20, comparison.Width * 0.02);
            var bySide = accepted
                .Where(x => Math.Abs(x.Offset) > sideTol)
                .GroupBy(x => Math.Sign(x.Offset))
                .Select(g => g
                    .OrderBy(x => Math.Abs(x.OutwardDistance))
                    .ThenByDescending(x => x.AxisAlignment + x.ArcLineAlignment)
                    .First())
                .ToList();

            List<RawEdgeTangencyCandidate> finalPool = bySide.Count > 0 ? bySide : accepted;
            RawEdgeTangencyCandidate selected = outwardSign > 0
                ? finalPool.OrderByDescending(x => x.Station)
                    .ThenByDescending(x => x.AxisAlignment + x.ArcLineAlignment).First()
                : finalPool.OrderBy(x => x.Station)
                    .ThenByDescending(x => x.AxisAlignment + x.ArcLineAlignment).First();

            station = selected.Station;
            source = "RAW_CAD_ARC_TO_LINE"
                + "|EDGE=" + selected.SourceHandle
                + "|SRCSEG=" + selected.SourceSegmentIndex.ToString(CultureInfo.InvariantCulture)
                + "|AXIS=" + selected.AxisAlignment.ToString("0.000", CultureInfo.InvariantCulture)
                + "|TAN=" + selected.ArcLineAlignment.ToString("0.000", CultureInfo.InvariantCulture)
                + "|KIND=" + selected.Kind;
            return true;
        }

        private Curve? TryOpenCurveByHandle(Database db, Transaction tr, string handle)
        {
            if (string.IsNullOrWhiteSpace(handle)) return null;
            try
            {
                ObjectId id = _geometry.FromHandle(db, handle.Trim());
                if (id.IsNull || !id.IsValid || id.IsErased) return null;
                return tr.GetObject(id, OpenMode.ForRead, false) as Curve;
            }
            catch
            {
                return null;
        ...
        private void CollectPolylineArcToLineTransitions(
            Database db,
            Transaction tr,
            Entity axis,
            ArmComparisonState comparison,
            Polyline edge,
            string sourceHandle,
            IEnumerable<int> preferredSegments,
            List<string> connectionHandles,
            double boundaryStation,
            int outwardSign,
            List<RawEdgeTangencyCandidate> output)
        {
            if (edge == null || edge.NumberOfVertices < 2) return;

            int segmentCount = edge.Closed ? edge.NumberOfVertices : edge.NumberOfVertices - 1;
            if (segmentCount <= 0) return;

            HashSet<int> preferred = new HashSet<int>((preferredSegments ?? Enumerable.Empty<int>())
                .Where(i => i >= 0 && i < segmentCount));

            IEnumerable<int> arcIndices = preferred.Count > 0
                ? preferred.Where(i => IsPolylineArcSegment(edge, i))
                : Enumerable.Range(0, segmentCount).Where(i => IsPolylineArcSegment(edge, i));

            foreach (int arcIndex in arcIndices.Distinct())
            {
                int startVertex = arcIndex;
                int endVertex = (arcIndex + 1) % edge.NumberOfVertices;

                int previousSegment = arcIndex - 1;
                if (previousSegment < 0 && edge.Closed) previousSegment = segmentCount - 1;
                if (TryGetBullhornEndpointTangent(edge, arcIndex, true, out Vector3d arcStartTangent))
                {
                    Vector3d previousTangent = Vector3d.XAxis;
                    bool hasStraight = previousSegment >= 0 && previousSegment < segmentCount &&
                        IsPolylineStraightSegment(edge, previousSegment) &&
                        TryGetPolylineLinearSegmentTangent(edge, previousSegment, out previousTangent);

                    if (hasStraight)
                    {
                        TryAddRawTangencyCandidate(
                            axis, comparison, edge.GetPoint3dAt(startVertex), arcStartTangent, previousTangent,
                            boundaryStation, outwardSign, sourceHandle, arcIndex, "POLYLINE_ARC_START_TO_SAME_LINE", output);
                    }
                    else if (TryFindConnectedStraightTangent(
                                 db, tr, edge.GetPoint3dAt(startVertex), sourceHandle, connectionHandles, out Vector3d externalStartTangent))
                    {
                        TryAddRawTangencyCandidate(
                            axis, comparison, edge.GetPoint3dAt(startVertex), arcStartTangent, externalStartTangent,
                            boundaryStation, outwardSign, sourceHandle, arcIndex, "POLYLINE_ARC_START_TO_EXTERNAL_LINE", output);
                    }
                }

                int nextSegment = arcIndex + 1;
                if (nextSegment >= segmentCount && edge.Closed) nextSegment = 0;
                if (TryGetBullhornEndpointTangent(edge, arcIndex, false, out Vector3d arcEndTangent))
                {
                    Vector3d nextTangent = Vector3d.XAxis;
                    bool hasStraight = nextSegment >= 0 && nextSegment < segmentCount &&
                        IsPolylineStraightSegment(edge, nextSegment) &&
                        TryGetPolylineLinearSegmentTangent(edge, nextSegment, out nextTangent);

                    if (hasStraight)
                    {
                        TryAddRawTangencyCandidate(
                            axis, comparison, edge.GetPoint3dAt(endVertex), arcEndTangent, nextTangent,
                            boundaryStation, outwardSign, sourceHandle, arcIndex, "POLYLINE_ARC_END_TO_SAME_LINE", output);
                    }
                    else if (TryFindConnectedStraightTangent(
                                 db, tr, edge.GetPoint3dAt(endVertex), sourceHandle, connectionHandles, out Vector3d externalEndTangent))
                    {
                        TryAddRawTangencyCandidate(
                            axis, comparison, edge.GetPoint3dAt(endVertex), arcEndTangent, externalEndTangent,
                            boundaryStation, outwardSign, sourceHandle, arcIndex, "POLYLINE_ARC_END_TO_EXTERNAL_LINE", output);
                    }
                }
            }
        }

        private void CollectStandaloneArcToLineTransitions(
            Database db,
            Transaction tr,
            Entity axis,
            ArmComparisonState comparison,
            ...
            List<RawEdgeTangencyCandidate> output)
        {
            foreach (bool atStart in new[] { true, false })
            {
                ...
                try
                {
                    point = atStart ? arc.StartPoint : arc.EndPoint;
                    double span = Math.Max(1e-8, arc.EndParam - arc.StartParam);
                    double eps = Math.Min(1e-5, span * 1e-4);
                    double param = atStart ? arc.StartParam + eps : arc.EndParam - eps;
                    arcTangent = arc.GetFirstDerivative(param);
                }
                catch
                {
                    continue;
                ...
                if (!TryFindConnectedStraightTangent(
                        db, tr, point, sourceHandle, connectionHandles, out Vector3d straightTangent))
                    continue;

                TryAddRawTangencyCandidate(
                    axis, comparison, point, arcTangent, straightTangent,
                    boundaryStation, outwardSign, sourceHandle, -1,
                    atStart ? "ARC_ENTITY_START_TO_LINE" : "ARC_ENTITY_END_TO_LINE", output);
            }
        }

        private bool TryFindConnectedStraightTangent(
            Database db,
            Transaction tr,
            Point3d point,
            string excludedHandle,
            IEnumerable<string> handles,
            out Vector3d tangent)
        {
            tangent = Vector3d.XAxis;
            const double connectionTolerance = 0.35;
            double bestDistance = double.MaxValue;
            Vector3d best = Vector3d.XAxis;
            bool found = false;

            foreach (string handle in handles ?? Enumerable.Empty<string>())
            {
                if (string.IsNullOrWhiteSpace(handle) ||
                    string.Equals(handle, excludedHandle, StringComparison.OrdinalIgnoreCase))
                    continue;

                Curve? curve = TryOpenCurveByHandle(db, tr, handle);
                ...
                if (curve is Line line)
                {
                    try
                    {
                        ...
                        double d = ToXy(point).DistanceTo(ToXy(closest));
                        Vector3d dir = new Vector3d(
                            line.EndPoint.X - line.StartPoint.X,
                            line.EndPoint.Y - line.StartPoint.Y,
                            0.0);
                        if (d <= connectionTolerance && dir.Length > 1e-9 && d < bestDistance)
                        {
                            bestDistance = d;
                            best = dir.GetNormal();
                            found = true;
                ...
                if (curve is Polyline pl)
                {
                    ...
                    {
                        if (!IsPolylineStraightSegment(pl, i) ||
                            !TryGetPolylineLinearSegmentTangent(pl, i, out Vector3d dir))
                            continue;

                        int j = (i + 1) % pl.NumberOfVertices;
                        try
                        {
                            using (var lineSegment = new Line(pl.GetPoint3dAt(i), pl.GetPoint3dAt(j)))
                            {
                                Point3d closest = lineSegment.GetClosestPointTo(point, false);
                                double d = ToXy(point).DistanceTo(ToXy(closest));
                                if (d <= connectionTolerance && d < bestDistance)
                                {
                                    bestDistance = d;
                                    best = dir;
                                    found = true;
        ...
        private void TryAddRawTangencyCandidate(
            Entity axis,
            ArmComparisonState comparison,
            Point3d point,
            Vector3d arcTangent,
            Vector3d straightTangent,
            double boundaryStation,
            int outwardSign,
            string sourceHandle,
            ...
            List<RawEdgeTangencyCandidate> output)
        {
            if (!_station.TryStationOffset(axis, point, out double candidateStation, out double offset))
                return;

            ResolveRoadSideWidths(comparison, out double leftWidth, out double rightWidth);
            double halfEnvelope = Math.Max(Math.Max(leftWidth, rightWidth), Math.Max(2.0, comparison.Width * 0.5));
            double maxLateral = halfEnvelope + Math.Max(1.5, comparison.Width * 0.15);
            if (Math.Abs(offset) > maxLateral)
                return;

            double searchRadius = Math.Max(8.0, Math.Min(40.0, Math.Max(comparison.Width * 2.5, 14.0)));
            if (Math.Abs(candidateStation - boundaryStation) > searchRadius)
                return;

            double outwardDistance = (candidateStation - boundaryStation) * outwardSign;
            double inwardTolerance = Math.Max(1.0, Math.Min(3.0, comparison.Width * 0.20));
            if (outwardDistance < -inwardTolerance)
                return;

            Vector3d arcDir = ToXyNormal(arcTangent);
            Vector3d lineDir = ToXyNormal(straightTangent);
            if (arcDir.Length <= 1e-9 || lineDir.Length <= 1e-9)
                return;

            Vector3d axisTangent;
            try
            {
                _station.PointAtStationOffset(axis, candidateStation, 0.0, out axisTangent);
            }
            catch
            {
                return;
            ...
            Vector3d axisDir = ToXyNormal(axisTangent);
            if (axisDir.Length <= 1e-9)
                return;

            double arcLineAlignment = Math.Abs(arcDir.DotProduct(lineDir));
            double axisAlignment = Math.Abs(lineDir.DotProduct(axisDir));
            arcLineAlignment = Math.Max(0.0, Math.Min(1.0, arcLineAlignment));
            axisAlignment = Math.Max(0.0, Math.Min(1.0, axisAlignment));

            output.Add(new RawEdgeTangencyCandidate
            {
                Station = candidateStation,
                Offset = offset,
                ArcLineAlignment = arcLineAlignment,
        ...
        private static bool IsPolylineArcSegment(Polyline pl, int segmentIndex)
        {
            try { return Math.Abs(pl.GetBulgeAt(segmentIndex)) > 1e-8; }
            catch { return false; }
        }

        private static bool IsPolylineStraightSegment(Polyline pl, int segmentIndex)
        {
            try { return Math.Abs(pl.GetBulgeAt(segmentIndex)) <= 1e-8; }
            catch { return false; }
        }

        private static bool TryGetPolylineLinearSegmentTangent(
            Polyline pl,
            int segmentIndex,
            out Vector3d tangent)
        {
            tangent = Vector3d.XAxis;
            if (pl == null || pl.NumberOfVertices < 2 || !IsPolylineStraightSegment(pl, segmentIndex))
                return false;
            int j = (segmentIndex + 1) % pl.NumberOfVertices;
            try
            {
                Point3d a = pl.GetPoint3dAt(segmentIndex);
                Point3d b = pl.GetPoint3dAt(j);
                Vector3d d = new Vector3d(b.X - a.X, b.Y - a.Y, 0.0);
        ...
        /// <summary>
        /// Xác định đúng điểm TIẾP TUYẾN CONG -> THẲNG của sừng bò thuộc approach hiện tại.
        ///
        /// Bản cũ chỉ gom mọi endpoint của các cung rồi lấy Max/Min station. Cách đó có thể
        /// chọn nhầm endpoint ở phía đường giao cắt, đặc biệt tại nút xiên hoặc polygon có
        /// nhiều sừng bò. Bản này kiểm tra hình học thực sự:
        /// 1) lấy tiếp tuyến của cung polygon tại từng endpoint;
        /// 2) chiếu endpoint lên TIM và lấy tiếp tuyến TIM tại station đó;
        /// 3) chỉ endpoint có hai tiếp tuyến gần song song mới là đầu cong chuyển sang
        ///    đoạn MÉP thẳng của approach;
        /// 4) lọc theo phía ngoài nút và khoảng offset mặt đường;
        /// 5) nếu hai mép có station tiếp tuyến lệch nhẹ, lấy mốc NGOÀI NHẤT để toàn bộ
        ///    vùng 7.3 bắt đầu sau khi cả hai sừng bò đã kết thúc.
        ///
        /// boundaryStation chỉ là fallback cho polygon cũ/fallback không có sừng bò.
        /// </summary>
        private double ResolveBullhornEndStation(
            Database db,
            ArmProjectState state,
            Entity axis,
            ArmComparisonState comparison,
            ...
            out string source)
        {
            source = "POLYGON_BOUNDARY_FALLBACK";

            // ƯU TIÊN TUYỆT ĐỐI: đọc điểm chuyển ARC/BULGE -> LINE từ chính MÉP CAD nguồn.
            // Polygon chỉ còn là vùng/topology; không được dùng vertex helper để quyết định
            // vị trí 7.3 nếu dữ liệu MÉP thật còn tồn tại.
            if (TryResolveRawCadBullhornTangencyStation(
                    db, state, axis, comparison, polygon, tr, boundaryStation, outwardSign,
                    out double rawStation, out string rawSource))
            {
                source = rawSource;
                return rawStation;
            }

            List<BoundaryRunInfo> runs = ReadBoundaryRuns(polygon, tr)
                .Where(x => x.Role.Equals("BULLHORN_EDGE", StringComparison.OrdinalIgnoreCase))
                .ToList();

            bool fromBoundaryRuns = runs.Count > 0;
            if (!fromBoundaryRuns)
            {
                // Tương thích polygon cũ: cạnh có bulge chính là biên cong sừng bò.
                for (int i = 0; i < polygon.NumberOfVertices; i++)
                {
                    try
                    {
                        if (Math.Abs(polygon.GetBulgeAt(i)) > 1e-8)
                            runs.Add(new BoundaryRunInfo { SegmentIndex = i, Role = "BULLHORN_EDGE" });
                    }
                    catch { }
                }
            }
            ...
            if (runs.Count == 0)
                return boundaryStation;

            ResolveRoadSideWidths(comparison, out double leftWidth, out double rightWidth);
            double halfEnvelope = Math.Max(
                Math.Max(leftWidth, rightWidth),
                Math.Max(2.0, comparison.Width * 0.5));
            double maxLateral = halfEnvelope + Math.Max(1.5, comparison.Width * 0.15);
            double searchRadius = Math.Max(
                8.0,
                Math.Min(35.0, Math.Max(comparison.Width * 2.0, 12.0)));

            bool hasCenterStation = _station.TryStationOffset(
                axis, PolygonCentroid(polygon), out double centerStation, out _);

            var rawCandidates = new List<BullhornTangencyCandidate>();

            foreach (BoundaryRunInfo run in runs)
            {
                if (run.SegmentIndex < 0 || run.SegmentIndex >= polygon.NumberOfVertices)
                    continue;

                int next = (run.SegmentIndex + 1) % polygon.NumberOfVertices;
                var endpoints = new[]
                {
                    new { Point = polygon.GetPoint3dAt(run.SegmentIndex), AtStart = true },
                    new { Point = polygon.GetPoint3dAt(next), AtStart = false }
                };

                foreach (var endpoint in endpoints)
                {
                    if (!_station.TryStationOffset(axis, endpoint.Point, out double station, out double offset))
                        continue;
                    if (Math.Abs(offset) > maxLateral)
                        continue;
                    if (Math.Abs(station - boundaryStation) > searchRadius)
                        continue;

                    double outwardDistance = (station - boundaryStation) * outwardSign;
                    double inwardTolerance = Math.Max(0.75, Math.Min(2.50, comparison.Width * 0.15));
                    if (outwardDistance < -inwardTolerance)
                        continue;

                    // Loại sừng bò thuộc approach đối diện của cùng TIM.
                    if (hasCenterStation && (station - centerStation) * outwardSign < -0.25)
                        continue;

                    if (!TryGetBullhornEndpointTangent(
                            polygon, run.SegmentIndex, endpoint.AtStart, out Vector3d bullhornTangent))
                        continue;

                    try
                    {
                        _station.PointAtStationOffset(axis, station, 0.0, out Vector3d axisTangent);
                        Vector3d a = ToXyNormal(bullhornTangent);
                        Vector3d b = ToXyNormal(axisTangent);
                        if (a.Length <= 1e-9 || b.Length <= 1e-9)
                            continue;

                        double alignment = Math.Abs(a.DotProduct(b));
                        alignment = Math.Max(0.0, Math.Min(1.0, alignment));

                        rawCandidates.Add(new BullhornTangencyCandidate
                        {
                            Station = station,
                            Offset = offset,
                            Alignment = alignment,
                            ...
                            AtStart = endpoint.AtStart,
                            SourceHandle = run.SourceHandle ?? string.Empty
                        });
                    }
                    catch { }
                }
            ...
            // Với mỗi cung sừng bò, chỉ giữ endpoint có tiếp tuyến song song TIM nhất.
            // Đây chính là thao tác loại đầu cung phía đường giao cắt.
            List<BullhornTangencyCandidate> perHorn = rawCandidates
                .GroupBy(x => x.SegmentIndex)
                .Select(g => g
                    .OrderByDescending(x => x.Alignment)
                    .ThenBy(x => Math.Abs(x.Station - boundaryStation))
                    .First())
                .ToList();

            double bestAlignment = perHorn.Max(x => x.Alignment);

            // Một tiếp tuyến đúng về lý thuyết có alignment ~= 1.0. Dùng 0.92 (~23 độ)
            // làm ngưỡng chính; nếu polygon/proxy bị xấp xỉ mạnh thì vẫn cho nhóm gần
            // bestAlignment nhất thay vì quay lại heuristic Max/Min endpoint mù.
            List<BullhornTangencyCandidate> aligned = perHorn
                .Where(x => x.Alignment >= 0.92)
                .ToList();

            if (aligned.Count == 0)
            {
                aligned = perHorn
                    .Where(x => x.Alignment >= Math.Max(0.75, bestAlignment - 0.035))
                    .ToList();
            }
            ...
            // Chỉ giữ cụm tiếp tuyến thuộc đúng approach gần boundaryStation.
            double nearest = aligned.Min(x => Math.Abs(x.Station - boundaryStation));
            double clusterTolerance = Math.Max(
                0.75,
                Math.Min(3.0, comparison.Width * 0.20));

            List<BullhornTangencyCandidate> local = aligned
                .Where(x => Math.Abs(x.Station - boundaryStation) <= nearest + clusterTolerance)
                .ToList();

            if (local.Count == 0)
                local = aligned;

            // Nếu hai mép chuyển cong->thẳng lệch station nhẹ, mốc ngoài nhất đảm bảo
            // vùng zebra không ăn trở lại bất kỳ phần cong nào.
            BullhornTangencyCandidate selected = outwardSign > 0
                ? local.OrderByDescending(x => x.Station).ThenByDescending(x => x.Alignment).First()
                : local.OrderBy(x => x.Station).ThenByDescending(x => x.Alignment).First();

            source = (fromBoundaryRuns ? "BOUNDARY_RUN" : "POLYGON_BULGE")
                + "_TRUE_TANGENCY"
                + "|SEG=" + selected.SegmentIndex.ToString(CultureInfo.InvariantCulture)
                + "|ALIGN=" + selected.Alignment.ToString("0.000", CultureInfo.InvariantCulture)
                + (string.IsNullOrWhiteSpace(selected.SourceHandle)
                    ? string.Empty
                    : "|EDGE=" + selected.SourceHandle);
        ...
        private bool TryGetBullhornEndpointTangent(
            Polyline polygon,
            int segmentIndex,
            bool atStart,
            out Vector3d tangent)
        {
            ...
            try
            {
                Point3d a = polygon.GetPoint3dAt(segmentIndex);
                Point3d b = polygon.GetPoint3dAt(next);
                double bulge = polygon.GetBulgeAt(segmentIndex);
                ...
                using (var segment = new Polyline(2))
                {
                    ...
                    double start = segment.StartParam;
                    double end = segment.EndParam;
                    double span = Math.Max(1e-6, end - start);
                    double epsilon = Math.Min(span * 1e-4, 1e-4);
                    double param = atStart
                        ? Math.Min(end, start + epsilon)
                        : Math.Max(start, end - epsilon);

                    Vector3d d = segment.GetFirstDerivative(param);
                    Vector3d xy = new Vector3d(d.X, d.Y, 0.0);
                    if (xy.Length <= 1e-9)
                        return false;

                    tangent = xy.GetNormal();
                    return true;
                }
            }
            catch
