...
{
    public sealed class CenterlineSelectionService
    {
        ...
        {
            var ids = _selection.SelectCurves(doc.Editor, tr, "Quét chọn TIM tuyến rồi nhấn Enter.", true);
            var handles = new List<string>();
            foreach (var id in ids) handles.Add(CurveGeometryHelper.Handle(id));
            state.SelectedTimHandles = handles;
            return handles;
        }
    }
}
